...
def create_demo_data():
    ...
    podrazdelenie = Podrazdelenie.objects.get(id=3)  # Линейные сотрудники
    ...
    # Создаем демо-объект
    demo_obj, created = Objekt.objects.get_or_create(
        nazvanie="Демо-объект: Строительство жилого дома",
        defaults={
            'data_nachala': date.today() - timedelta(days=30),
            'data_plan_zaversheniya': date.today() + timedelta(days=60),
            'status': 'в работе',
            'demo': True,
            'is_active': True,
            'otvetstvennyj': 'Демо Пользователь'
        }
    )
    ...
    for spec_name in specialnosti:
        ...
        resurs_po_obj, created = ResursyPoObjektu.objects.get_or_create(
            ...
            resurs=resurs,
            defaults={
                'kolichestvo': 160,  # 160 часов в месяц
                'cena': random.randint(800, 1500)  # Ставка от 800 до 1500 руб/час
            }
        )
    ...
    for i in range(30):
        ...
        for emp in created_employees:
            # Случайно выбираем, работал ли сотрудник в этот день
            if random.choice([True, False, True]):  # 66% вероятность работы
                hours = random.randint(6, 10)  # От 6 до 10 часов
                kpi = round(random.uniform(0.8, 1.2), 2)  # KPI от 0.8 до 1.2
