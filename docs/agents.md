...
для блока "Сотрудники по объекту"
сделай 2 колонки:
Основное подразделение (для сотрудников sotrudniki_specialnost, kategoriya = 1)
Строительное управление (для сотрудников sotrudniki_specialnost, kategoriya = 2)
...
<input type="number" name="expense_quantity[]">  value сделай с 1м знаком после запятой 
для <input type="number" name="expense_price[]"> value сделай целым числом
подписать руб,  шаг сделать step="1"


удали <select name="expense_category[]" class="expense-category" 
...
<input type="number" name="expense_price[]">
