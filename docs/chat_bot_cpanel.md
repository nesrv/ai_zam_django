...
## 🚫 Способ 2: Polling (НЕ РЕКОМЕНДУЕТСЯ на cPanel)

`python-telegram-bot` с `updater.start_polling()` требует фоновый процесс — на cPanel его сложно поддерживать, потому **используй Webhook.**

---

## ✅ Проверка

После установки Webhook отправь сообщение боту — он должен ответить.
...
| Проблема                     | Решение                                                       |
| ---------------------------- | ------------------------------------------------------------- |
| ❌ 403 CSRF                   | Используй `@csrf_exempt` в представлении                      |
| ❌ 500 Internal Server Error  | Проверь логи в `~/logs/` или через Error Logs в cPanel        |
| ❌ SSL/TLS ошибка при Webhook | Убедись, что сайт доступен по HTTPS (используй Let's Encrypt) |
| ❌ Django не запускается      | Проверь `passenger_wsgi.py`, окружение и `ALLOWED_HOSTS`      |
